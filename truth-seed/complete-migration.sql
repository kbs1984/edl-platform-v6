...
-- Project: Debate Application with EDL Customizations
-- =============================================
...
-- =============================================
-- STEP 5: EDL Customization - Add call_sign
-- =============================================
-- Add call_sign column if student table exists but column doesn't
DO $$
BEGIN
    IF EXISTS (
        ...
        ADD COLUMN call_sign text UNIQUE NOT NULL DEFAULT 'TEMP_' || gen_random_uuid()::text;
        
        -- Remove default after column is added
        ALTER TABLE public.student ALTER COLUMN call_sign DROP DEFAULT;
...
-- =============================================
-- STEP 10: Additional Indexes (Optional)
-- =============================================
...
-- =============================================
-- STEP 11: Fix Character Encoding Issues
-- =============================================
-- Update any Korean text that might be corrupted
UPDATE pg_proc 
SET prosrc = REPLACE(prosrc, 'ì—…ë°ì´íŠ¸', 'update')
WHERE prosrc LIKE '%ì—…ë°ì´íŠ¸%';

UPDATE pg_proc 
SET prosrc = REPLACE(prosrc, 'ì‚½ìž…', 'insert')
WHERE prosrc LIKE '%ì‚½ìž…%';
