...
status: "applied"
created: "2025-08-27"
title: "Deployed Migration: batch-04-constraints-fixed"
purpose: "Applied database migration - foreign keys"
...
-- =============================================
-- Batch 04: Foreign Key Constraints (Fixed - No Duplicates)
-- Session 00051
-- Purpose: Add all foreign key relationships
-- Dependencies: Batch 03, 03b, 03c, 03d (all table structure must exist)
-- =============================================
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'message_room_id_fkey') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'participant_room_id_fkey') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'participant_student_id_fkey') THEN
        ALTER TABLE ONLY chat.participant
            ADD CONSTRAINT participant_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.profile(id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'room_guild_id_fkey') THEN
        ALTER TABLE ONLY chat.room
            ADD CONSTRAINT room_guild_id_fkey FOREIGN KEY (guild_id) REFERENCES public.guild(id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'room_team_id_fkey') THEN
        ALTER TABLE ONLY chat.room
...
--
-- Expected: ~52 foreign key constraints (after removing duplicates)
